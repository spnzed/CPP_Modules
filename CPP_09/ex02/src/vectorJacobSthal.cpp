...
static int search_pos(int num, std::vector<int>& chain, int size) {
...
static void big_size(std::vector<int>& stack, std::vector<int>& chain_B, std::vector<int> j_numbers,
    ...
    for (int i = 1; i < it_end; i++) {
        ...
        for (int j = limit - 1; j > prev && j <= end; j -= (group_size * 2)) {
            ...
            std::vector<int>::iterator first = stack.begin() + j + 1;
            std::vector<int>::iterator second = stack.begin() + j + group_size + 1;
...
static void min_size(std::vector<int>& stack, std::vector<int>& chain_B, std::vector<int> j_numbers,
            int it_end, int end, int group_size) {
    ...
    for (int i = 1; i < it_end; i += 1) {
        ...
        for (int j = limit; j > prev && j <= end; j -= (group_size * 2)) {
            ...
            std::vector<int>::iterator first = stack.begin() + j;
...
void insertWithJacobsthal(std::vector<int>& stack, int end, int group_size) {
    ...
    std::vector<int> chain_B = getStack(stack, int(stack.size()), group_size, 1);
